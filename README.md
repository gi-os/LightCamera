...
the CI run number — so the release published from the current `main` is `v2.38.x`
(2026-08-01), the one whose send picker can reach a group. See
...
| Version | Date | Notes |
|---|---|---|
| `v2.38.x` (pending) | — | **The send picker can reach a group.** "Who is this photograph for?" had one answer it could never give: an address book knows about people, and a group iMessage is not a person — it is a chat room on the Mac, identified by a guid like `iMessage;+;chat684…`, with no number and no contact row. Groups now sit above the contacts, read read-only out of [LightChat](https://github.com/gi-os/LightChat)'s new `com.gios.lightchat.chats` provider with their names already resolved through the BlueBubbles address book, so a group reads the same in both apps. Five at rest — each one is a contact pushed off the screen — and every match once you type; a query containing a digit excludes them all, because in the contact half digits search phone numbers and a group has no number to search. **Sending splits by kind and the split is the point.** A person goes out as `ACTION_SEND` with the AOSP `address` extra and can still fall through to any messaging app that understands it; a group goes to LightChat alone with the guid in a `chat_guid` extra, with no chooser fallback, because every other receiver would ignore the extra and drop the photographs into a thread the user never chose — the same lie the address path was fixed for, and worse, since a group has no address to fall back on. Groups are not recorded as recents: they already sort by their own last activity, which is a better signal than a six-slot list. Needs LightChat v1.2; without it there are simply no groups and the picker is unchanged. |
